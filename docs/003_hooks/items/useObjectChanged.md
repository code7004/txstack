...
```
docs/README.md 와 docs/003_hooks/items/useObjectChanged.md 를 읽고 003-useObjectChanged-S1 부터 진행해줘.
```
...
| 단계 | 내용                            | job ID                    | 상태 | 비고                             |
| ---- | ------------------------------- | ------------------------- | ---- | -------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝 | `003-useObjectChanged-S1` |      |                                  |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖     | `003-useObjectChanged-S2` |      |                                  |
| `S3` | 테스트 🤖                       | `003-useObjectChanged-S3` |      |                                  |
| `S4` | 스토리북                        | —                         | —    | `ui` 패키지가 아니므로 해당 없음 |
| `S5` | 문서 사이트 (예제·코드) 🤖      | `003-useObjectChanged-S5` |      |                                  |
| `S6` | Claude 가이드 🤖                | `003-useObjectChanged-S6` |      |                                  |

## 1. 현재 코드 인벤토리 (2026-08-25)

| 항목      | 값                                       |
| --------- | ---------------------------------------- |
| 위치      | `packages/hooks/src/useObjectChanged.ts` |
| 코드 행수 | 66행                                     |
| export    | `useObjectChanged`                       |

### 착수 전에 알아야 할 것

- **존재 이유부터 판단한다.** 요청 범위(`useUrlQuery`)에 없다 — 유지/폐기 후보
