...
**미배포.** npm 에 아무것도 올라가지 않았고, **git 커밋도 0건**이다 (사용자가 이동 후 커밋 예정).
...
| 항목                                     | 내용                                                                                                                                                                                                                     |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`TxForm.DayPicker` API** (⚠ 가장 중요) | subpath 분리 때문에 `TxForm.DayPicker` → `TxFormDayPicker`(`@txstack/ui/daypicker`) 로 바뀌었다. 유지하려면 `react-day-picker` 를 **필수** peer 로 돌려야 한다 (날짜를 안 쓰는 소비자도 설치). **어느 쪽으로 갈지 미정** |
| git remote                               | GitHub 저장소 주소 미정. 정해지면 각 `package.json` 에 `repository` 필드 추가 필요 (현재 비어 있음)                                                                                                                      |
| npm org                                  | npmjs.com 에 org(또는 사용자명) **`txstack`** 생성 필요. P4 배포 직전까지만 있으면 된다                                                                                                                                  |

### 6-2. 다음 Phase

- **P4 — npm 배포 v0.1.0-alpha**
  1. npm org `txstack` 생성 · `npm login`
  2. `package.json` 에 `repository` 필드 추가
