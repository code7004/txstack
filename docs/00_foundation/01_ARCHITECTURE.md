...
| 항목                                                                        | 어디서 결정하나                       |
| --------------------------------------------------------------------------- | ------------------------------------- |
| `@txstack/ui` 의 커스터마이징 방식 (theme 객체 / className 병합 / CSS 변수) | `001_ui/20_design.md`                 |
| Tailwind v4 `@source` purge 제약을 소비자에게 계속 요구할 것인가            | `001_ui/10_requirements.md`           |
...
| 문서 사이트(903)를 Storybook 과 통합할 것인가                               | `903_docs_site/10_requirements.md`    |
| Claude 가이드의 파일명·원본 위치 (패키지 동봉은 확정)                       | `904_claude_guide/10_requirements.md` |
