...
### 문서 — 작업 항목 문서에는 **순서 번호**를 붙인다

```
docs/001_ui/components/01_TxSpinner.md   …   26_TxDayPicker.md
docs/004_network/items/01_types.md       …   05_singleton.md
```

| 규칙                                 | 이유                                                             |
| ------------------------------------ | ---------------------------------------------------------------- |
| `NN_<항목이름>.md` (두 자리)         | 폴더를 열면 **다음에 뭘 할 차례인지** 바로 보인다                |
| 번호는 그 트랙 `30_tasks.md` 의 순서 | 보드와 폴더가 어긋나면 둘 다 못 믿게 된다                        |
| **번호를 나중에 당기지 않는다**      | 폐기(`❌`)돼도 비워 둔다. 당기면 기존 링크·커밋이 딴 걸 가리킨다 |

트랙 문서 자체(`10_requirements` · `20_design` · `30_tasks` · `40_verification`)의 번호는
**순서가 아니라 종류**다. 이건 그대로 둔다.
...
> 그때까지 **새로 쓰는 코드는 무조건 `I` 없는 쪽**이다. 근거와 버린 대안: [001_ui/components/01_TxSpinner.md §5 Q1](../001_ui/components/01_TxSpinner.md#q1--i-접두-폐지)
