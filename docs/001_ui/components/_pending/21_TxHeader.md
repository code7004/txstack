...
```
docs/README.md 와 docs/001_ui/components/21_TxHeader.md 를 읽고 001-TxHeader-S1 부터 진행해줘.
```
...
| 단계 | 내용                                           | job ID            | 상태 | 비고                                              |
| ---- | ---------------------------------------------- | ----------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                | `001-TxHeader-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                    | `001-TxHeader-S2` |      |                                                   |
| `S3` | 테스트 🤖                                      | `001-TxHeader-S3` |      |                                                   |
| `S4` | 스토리북 🤖                                    | `001-TxHeader-S4` |      | 기존 스토리 있음 — 양식에 맞춰 개편               |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —                 |      | 통과하면 S5 로. 고칠 게 나오면 S2~S4 를 다시 돈다 |
| `S5` | 문서 사이트 🤖                                 | `001-TxHeader-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                               | `001-TxHeader-S6` |      |                                                   |
...
| 항목           | 값                                                                         |
| -------------- | -------------------------------------------------------------------------- |
| 위치           | `packages/ui/src/TxHeader/`                                                |
| 코드 행수      | 28행 (스토리 제외)                                                         |
| 파일           | `TxHeader.stories.tsx` · `TxHeader.theme.ts` · `TxHeader.tsx` · `index.ts` |
| named export   | `TxHeaderTheme` · `TxHeader`                                               |
| default export | 없음                                                                       |
| props 타입     | `ITxHeader`                                                                |
| `.theme.ts`    | 있음                                                                       |
| `.types.ts`    | **없음**                                                                   |
