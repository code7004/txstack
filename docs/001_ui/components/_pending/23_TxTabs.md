...
```
docs/README.md 와 docs/001_ui/components/23_TxTabs.md 를 읽고 001-TxTabs-S1 부터 진행해줘.
```
...
| 단계 | 내용                                           | job ID          | 상태 | 비고                                              |
| ---- | ---------------------------------------------- | --------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                | `001-TxTabs-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                    | `001-TxTabs-S2` |      |                                                   |
| `S3` | 테스트 🤖                                      | `001-TxTabs-S3` |      |                                                   |
| `S4` | 스토리북 🤖                                    | `001-TxTabs-S4` |      | 기존 스토리 있음 — 양식에 맞춰 개편               |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —               |      | 통과하면 S5 로. 고칠 게 나오면 S2~S4 를 다시 돈다 |
| `S5` | 문서 사이트 🤖                                 | `001-TxTabs-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                               | `001-TxTabs-S6` |      |                                                   |
...
| 항목           | 값                                                                                       |
| -------------- | ---------------------------------------------------------------------------------------- |
| 위치           | `packages/ui/src/TxTabs/`                                                                |
| 코드 행수      | 111행 (스토리 제외)                                                                      |
| 파일           | `TxTabs.stories.tsx` · `TxTabs.theme.ts` · `TxTabs.tsx` · `TxTabs.types.ts` · `index.ts` |
| named export   | `TxTabsTheme` · `TxTabs`                                                                 |
| default export | 없음                                                                                     |
| props 타입     | `ITxTabRenderHeadProps` · `ITxTabs`                                                      |
| `.theme.ts`    | 있음                                                                                     |
| `.types.ts`    | 있음                                                                                     |
| 테스트         | **없음**                                                                                 |
| `data-tag`     | 있음                                                                                     |
| 스토리         | `Overlay/TxTabs` (스토리 4개)                                                            |

### 착수 전에 알아야 할 것

- 스토리 그룹이 `Overlay/` — 탭이 오버레이인지 재분류 검토
