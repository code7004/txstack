...
| 단계 | 내용                                           | job ID            | 상태 | 비고                                              |
| ---- | ---------------------------------------------- | ----------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                | `001-TxButton-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                    | `001-TxButton-S2` |      |                                                   |
| `S3` | 테스트 🤖                                      | `001-TxButton-S3` |      |                                                   |
| `S4` | 스토리북 🤖                                    | `001-TxButton-S4` |      | 기존 스토리 있음 — 양식에 맞춰 개편               |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —                 |      | 통과하면 S5 로. 고칠 게 나오면 S2~S4 를 다시 돈다 |
| `S5` | 문서 사이트 🤖                                 | `001-TxButton-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                               | `001-TxButton-S6` |      |                                                   |
...
| 항목           | 값                                                         |
| -------------- | ---------------------------------------------------------- |
| 위치           | `packages/ui/src/TxButton/`                                |
| 코드 행수      | 134행 (스토리 제외)                                        |
| 파일           | `TxButton.stories.tsx` · `TxButton.theme.ts` · `index.tsx` |
| named export   | `TxButton` · `TxButtonTheme`                               |
| default export | 없음                                                       |
| props 타입     | `ITxButtonProps`                                           |
| `.theme.ts`    | 있음                                                       |
| `.types.ts`    | **없음**                                                   |
| 테스트         | **없음**                                                   |
| `data-tag`     | 있음                                                       |
| 스토리         | `Form/TxButton` (스토리 6개)                               |

### 착수 전에 알아야 할 것

- `.types.ts` 없이 props 가 구현 파일에 인라인
- `loading = <TxSpinner />` 기본값 — TxSpinner 의 D1 결함이 여기로 전파된다
- **파일럿 2차.** 커스터마이징 방식·props·콜백 이름 규칙을 여기서 확정한다
