...
`TxButton.test.tsx` (20개) · `TxButton.stories.tsx` (`Form/TxButton`, 5개)
...
| `S4` | 스토리북 🤖                                    | `001-TxButton-S4` | ✅   | 스토리 5개(플레이그라운드 포함) → §12                        |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —                 | 🔄   | **여기서 멈춰 있다.** `pnpm storybook:dev` → `Form/TxButton` |
...
`Form/TxButton` 이 실렸다 (`READY` 목록에 추가). 봐 주십사 하는 것 —

| 스토리           | 눈여겨볼 것                                                                      |
| ---------------- | -------------------------------------------------------------------------------- |
| `플레이그라운드` | 기본 모습이 쓸 만한가                                                            |
| `Variant`        | **`text` 에 여백이 생겼다.** 예전엔 여백이 없었다 — 이게 더 나은가               |
| `로딩`           | 동기 버튼이 안 잠기는 게 맞는가. 스피너 위치·크기                                |
| `폼_안에서`      | `type` 기본값이 바뀐 게 실제로 맞게 도는가                                       |
| `커스터마이징`   | **3단 병합이 이해되는 모양인가.** 이게 26종에 퍼질 방식이라 여기가 제일 중요하다 |

**`TxThemeProvider` 의 이름과 사용법**도 판단거리다. 26종에 퍼지기 전에 바꾸는 게 싸다.
...
`Form/TxButton` — 5개. TxSpinner S4 에서 만든 양식을 그대로 적용했다.
...
| `커스터마이징`   | className / theme / **Provider** 3단을 한 화면에 | 꺼짐     |
