...
> 상태: **S1·S2 완료 (2026-08-25).** 이 문서가 `TxSpinner` 의 단일 진실 공급원이다.

현재 코드: `packages/ui/src/TxSpinner/TxSpinner.tsx` (구현 + props 타입) · `index.ts` (재수출) ·
`TxSpinner.stories.tsx` (S4 에서 개편) · **테스트 없음 (S3)**
...
| `S3` | 테스트 🤖                       | `001-TxSpinner-S3` |      | T1. `902` 양식을 여기서 만든다                          |
...
| ID  | 내용                                                                                        |
| --- | ------------------------------------------------------------------------------------------- |
| T1  | **테스트 없음.** `ui` 전체에 유틸 테스트 1개뿐이라 선례도 없다 → `902` 양식을 여기서 만든다 |

S3 에서 최소한 이것들을 못박는다. **고친 결함은 회귀 테스트로 남긴다.**

1. `size` 가 **width 와 height 둘 다**에 적용된다 (D1)
2. `className` 을 줘도 `animate-spin` 이 남는다 (D2)
3. `decorative` 가 켜지면 `role`·`aria-label` 이 없고 `aria-hidden` 이 있다 (A2)
4. `aria-label` 을 주면 기본 `"Loading"` 을 이긴다 (A1)
...
`001-TxSpinner-S3` — 테스트. `902` 양식을 여기서 만든다. §4 "테스트" 의 4항목이 최소치이고, **D2 가 여기서 처음 검증된다.**
