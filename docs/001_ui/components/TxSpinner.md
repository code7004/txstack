...
> 상태: **S1~S4 완료 (2026-08-25). 지금 🧑 사용자 확인 게이트에서 대기 중이다.**
> 확인이 끝나야 S5(문서 사이트) · S6(Claude 가이드) 로 넘어간다 — 2차 개선에서 바뀔 것을 미리 문서화하지 않는다.
...
`Feedback/TxSpinner` 하나만 실려 있다. 아래는 **봐 주십사 하는 것**이지 체크리스트는 아니다 —
"내가 원하던 모양인가"가 판정 기준이고, 그건 목록으로 못 적는다.

| 스토리           | 눈여겨볼 것                                                                                   |
| ---------------- | --------------------------------------------------------------------------------------------- |
| `기본`           | **기본 크기 `1em` 이 실제로 쓸 만한가.** 이전 `2em` 보다 작아졌다 — 여기가 제일 뒤집힐 만하다 |
| `크기`           | 폰트 크기 상속이 직관적인가. `size` 와 `className` 두 갈래가 헷갈리지 않는가                  |
| `색`             | `currentColor` 상속만으로 충분한가                                                            |
| `장식용`         | 버튼 안 스피너의 크기·위치가 맞는가                                                           |
| `안내_문구_교체` | 기본값이 영어(`"Loading"`)인 게 괜찮은가                                                      |

**12스포크 모양 자체**도 판단거리다 — §5 Q4 에서 "더 작은 원호로 교체"를 버린 대안으로 뒀는데,
겉모습이 마음에 안 들면 지금이 바꿀 자리다 (번들 1.4KB 절감은 덤).

확인 결과에 따라:

- **통과** → `001-TxSpinner-S5`(문서 사이트). 다만 `903` 은 코드가 아예 없고 Storybook 과의
  역할 분담(`901-05`·`903`)이 미결이라, 사이트를 무엇으로 세울지가 🤝 로 먼저 걸린다
- **2차 개선** → S2 로 돌아간다. 테스트·스토리도 함께 갱신하고 changeset 을 다시 쓴다
