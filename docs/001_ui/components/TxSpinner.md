...
> **`S1` 은 유효하고 `S2`~`S4` 를 다시 돈다.** `S5` 는 `903` 미정으로 ⏸ 그대로다.
...
| `S2` | 구현 = 감사 결과 반영 🤖                       | `001-TxSpinner-S2` | ↩    | 1차 완료(D1–D5·A1–A3·C1~C7) → §8. **CSS 로 다시 쓴다** |
| `S3` | 테스트 🤖                                      | `001-TxSpinner-S3` | ↩    | 1차 15개 → §11. 클래스 문자열을 짚는 테스트가 바뀐다   |
| `S4` | 스토리북 🤖                                    | `001-TxSpinner-S4` | ↩    | 1차 5개 → §12. 커스터마이징 스토리가 바뀐다            |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —                  | ↩    | 1차 통과(2026-08-25) → §12. 2차에서 다시 받는다        |
| `S5` | 문서 사이트 🤖                                 | `001-TxSpinner-S5` | ⏸    | **`903` 도구 미정으로 보류.** S6 를 먼저 했다 → §15    |
| `S6` | 에이전트 가이드 🤖                             | `001-TxSpinner-S6` | ↩    | 1차 작성 → §14. `@source`·클래스 안내가 통째로 바뀐다  |

> **2026-08-25 — 게이트를 `↩` 로 되돌렸다.** [20_design](../20_design.md) 에서 **스타일을 Tailwind 클래스
> 문자열에서 자체 CSS 로** 바꾸기로 했다. `TxSpinner` 은 그 방침으로 `S2`~`S4` 를 다시 돈다.
>
> **`S1`(§1~§7)은 그대로 유효하다** — 목적·공개 API·접근성·범위 밖 판정은 스타일 방식과 무관하다.
> 아래 `§8` 이후의 처리·검증 기록은 **1차의 기록으로 남긴다.** 지우지 않는다 — 왜 이렇게 왔는지가 거기 있다.
...
- **`001-TxSpinner-S2`(2차)** — [20_design](../20_design.md) 대로 `TxSpinner.css` 를 만들고 클래스 문자열을 걷어낸다
