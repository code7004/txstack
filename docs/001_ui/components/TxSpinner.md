...
> 상태: **S1~S4 + 🧑 사용자 확인 통과 (2026-08-25).** 남은 것은 S5(문서 사이트) · S6(Claude 가이드).
> **공개 API 는 여기서 확정됐다.** 이제 문서화해도 안전하다.
...
🤝 결정으로 먼저 걸린다.** Storybook 과의 역할 분담(`901-05` · `903`)이 여기서 갈린다.

§6 예제와 §12 스토리가 이미 있으므로, 사이트가 그걸 **다시 쓰는지 참조하는지**가 첫 질문이다.
