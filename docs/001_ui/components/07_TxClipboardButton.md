...
```
docs/README.md 와 docs/001_ui/components/07_TxClipboardButton.md 를 읽고 001-TxClipboardButton-S1 부터 진행해줘.
```
...
| 단계 | 내용                                           | job ID                     | 상태 | 비고                                              |
| ---- | ---------------------------------------------- | -------------------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                | `001-TxClipboardButton-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                    | `001-TxClipboardButton-S2` |      |                                                   |
| `S3` | 테스트 🤖                                      | `001-TxClipboardButton-S3` |      |                                                   |
| `S4` | 스토리북 🤖                                    | `001-TxClipboardButton-S4` |      | 기존 스토리 있음 — 양식에 맞춰 개편               |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —                          |      | 통과하면 S5 로. 고칠 게 나오면 S2~S4 를 다시 돈다 |
| `S5` | 문서 사이트 🤖                                 | `001-TxClipboardButton-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                               | `001-TxClipboardButton-S6` |      |                                                   |
...
| 항목           | 값                                            |
| -------------- | --------------------------------------------- |
| 위치           | `packages/ui/src/TxClipboardButton/`          |
| 코드 행수      | 6행 (스토리 제외)                             |
| 파일           | `TxClipboardButton.stories.tsx` · `index.tsx` |
| named export   | 없음                                          |
...
| 테스트         | **없음**                                      |
| `data-tag`     | **없음**                                      |
| 스토리         | `Feedback/TxClipboardButton` (스토리 2개)     |

### 착수 전에 알아야 할 것

- 6행. 구현이 `index.tsx` 안에 있고 default export
- `data-tag` 없음
- 너무 작다 — 독립 컴포넌트로 유지할지 판단 필요
