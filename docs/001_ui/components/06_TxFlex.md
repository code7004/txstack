...
```
docs/README.md 와 docs/001_ui/components/06_TxFlex.md 를 읽고 001-TxFlex-S1 부터 진행해줘.
```
...
| 단계 | 내용                                           | job ID          | 상태 | 비고                                              |
| ---- | ---------------------------------------------- | --------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                | `001-TxFlex-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                    | `001-TxFlex-S2` |      |                                                   |
| `S3` | 테스트 🤖                                      | `001-TxFlex-S3` |      |                                                   |
| `S4` | 스토리북 🤖                                    | `001-TxFlex-S4` |      | 기존 스토리 있음 — 양식에 맞춰 개편               |
| 🧑   | **사용자 확인** — Storybook 에서 직접 만져본다 | —               |      | 통과하면 S5 로. 고칠 게 나오면 S2~S4 를 다시 돈다 |
| `S5` | 문서 사이트 🤖                                 | `001-TxFlex-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                               | `001-TxFlex-S6` |      |                                                   |
...
| 항목           | 값                                 |
| -------------- | ---------------------------------- |
| 위치           | `packages/ui/src/TxFlex/`          |
| 코드 행수      | 7행 (스토리 제외)                  |
| 파일           | `TxFlex.stories.tsx` · `index.tsx` |
| named export   | `TxFlex`                           |
| default export | 없음                               |
| props 타입     | **공개 타입 없음**                 |
| `.theme.ts`    | **없음**                           |
...
| 테스트         | **없음**                           |
| `data-tag`     | 있음                               |
| 스토리         | `Layout/TxFlex` (스토리 3개)       |

### 착수 전에 알아야 할 것

- 7행. 유지할 가치가 있는지 판단 필요 (`div className="flex"` 와의 차이)
