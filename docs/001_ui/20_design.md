...
| **Sass 로 쓴다** (전처리기)                   | 아래 §2-1. **간판 기능이 여기서는 쓰면 안 되는 도구다**                                                           |
...
## 2-1. 소스도 **평문 CSS 로 쓴다** — Sass 를 넣지 않는다 (2026-08-26)

**라이브러리 소스는 `.css` 다.** Sass·Less 같은 전처리기를 넣지 않는다.

### 이건 소비자와 무관한 결정이다

먼저 갈라 둔다. **배포물은 어느 쪽을 골라도 CSS 다** — 전처리기는 빌드타임 도구라
`dist/styles.css` 는 똑같이 나온다. 그러니 이건 "우리가 뭘로 쓰느냐" 만의 문제고,
**`R4`(소비자가 Sass 를 써도 된다)와는 아무 관계가 없다.** 그쪽은 그대로 유효하다.

### 왜 안 넣나 — Sass 의 간판 기능이 정확히 금지 사항이다

Sass 변수(`$primary`)와 `color.adjust()` 는 **빌드타임에 값을 굳힌다.**
그런데 이 라이브러리가 파는 것 자체가 **런타임 오버라이드**다 — `:root` 한 줄로 `hover`·포커스 링·
다크모드가 전부 따라오는 것(§4·§5).

```scss
// 이렇게 하는 순간 §5-1 의 상태 레이어가 무너진다
--tx-button-bg-hover: #{color.adjust($primary, $lightness: -10%)};
```

소비자가 `--tx-color-primary` 만 보라색으로 바꾸면 **hover 는 파란색으로 남는다.** 빌드 때 계산이 끝났으니까.

**이건 가정이 아니라 실제로 났던 결함이다** (`TxButton` D8 → §5-1). 짝 토큰으로 들고 있다가 같은 증상을 만났고,
`color.adjust()` 를 썼어도 결과는 똑같았을 것이다. **고친 방법이 `color-mix()` — 런타임에 계산하는 것**이었다.
Sass 로는 그 수정 자체가 불가능하다.

### 나머지 기능은 값이 작다

| Sass 가 주는 것   | 여기서                                                                |
| ----------------- | --------------------------------------------------------------------- |
| 변수 · color 함수 | **쓰면 안 된다** (위)                                                 |
| 중첩              | 네이티브 CSS 로 된다. 게다가 **지금 옮긴 두 파일 다 중첩을 안 쓴다**  |
| mixin · 반복      | 유일하게 실질적인 값. 다만 표본이 2종이라 **판단할 근거가 아직 없다** |
| 파셜 · `@use`     | `styles.css` 의 `@import` 색인이 이미 그 일을 한다 (`build-css.mjs`)  |

대신 치르는 비용: 의존과 빌드 단계가 하나 늘고, **CSS 계약 테스트가 흐려진다.**
지금 테스트는 `TxButton.css` 를 **우리가 쓴 그대로** 읽는다. 컴파일 산출물을 읽게 되면
"쓴 것" 이 아니라 "나온 것" 을 검사하게 되어 D2·D7 가드가 한 겹 약해진다.

### 언제 다시 보나

**8종쯤 옮긴 뒤 반복이 실제로 쌓였을 때.** 포커스 링·`disabled`·transition 이 26번 복붙되고 있으면 신호다.
그때도 **첫 수단은 Sass 가 아니라 공용 파일 하나**(`tx-base.css`)다. 그걸로 안 되면 그때 전처리기를 논한다.
...
**값은 토큰으로만 바꾼다.** 그래야 `hover`·`pressed`·`focus`·`dark` 가 **저절로 따라온다** —
상태별 클래스를 하나씩 덮던 문제가 여기서 사라진다.

> 이게 진짜로 성립하려면 **상태 색이 파생이어야 한다.** 짝 토큰(`--tx-color-primary-hover`)을
> 두면 이 문장이 거짓이 된다 — 실제로 한 번 거짓이었다. → §5-1

### `className` 이 이기게 하는 장치 — **캐스케이드 레이어** (2026-08-26)

**우리 CSS 는 전부 `tx` 레이어 안에 넣는다.** 이게 없으면 위 표의 두 번째·세 번째 줄이 거짓말이 된다.

레이어에 없는 CSS 는 레이어에 있는 CSS 를 **특이도와 소스 순서에 상관없이 무조건 이긴다.**
그래서 라이브러리 CSS 가 레이어 밖에 있으면 소비자가 `className` 으로 무엇을 주든
`.tx-button` 이 정한 속성은 꿈쩍도 하지 않는다 — Tailwind 유틸리티가 `@layer utilities` 안에 있기 때문이다.

> **`TxButton` 2차 실측에서 잡혔다.** `className="rounded-full px-6 shadow-lg"` 가 **셋 다 무시됐다.**
> `w-40`·`truncate` 는 먹었는데, 그건 우리가 안 정하는 속성이라서다 —
> **되는 속성과 안 되는 속성이 규칙 없이 갈리는 상태**였다.
> 1차에는 `tailwind-merge` 가 한 클래스 문자열 안에서 충돌을 풀어줘 이 구멍이 안 보였다.

| 소비자가 쓰는 것              | 필요한 것                                                |
| ----------------------------- | -------------------------------------------------------- |
| 순수 CSS · Sass · CSS Modules | **없음.** 레이어를 안 쓰는 CSS 가 항상 이긴다            |
| Tailwind                      | 레이어 순서 한 줄 — `tx` 를 `base` 와 `utilities` 사이에 |
...
**`tx` 는 preflight(`base`) 뒤, 유틸리티 앞이어야 한다.** 앞에 두면 preflight 가 버튼의 배경과
여백을 지우고, 뒤에 두면 `className` 이 다시 안 먹는다. 실물로 둘 다 확인했다.

- 배포본은 `@import … layer(tx)` 를 펼치면서 `@layer tx { … }` 블록으로 감싼다 (`scripts/build-css.mjs`)
- `:where()` 로 특이도를 0 으로 만드는 대안도 있지만 **레이어 문제를 못 푼다** —
  비레이어가 레이어를 이기는 건 특이도와 무관한 규칙이라, 유틸리티가 여전히 진다
...
전역 토큰은 `packages/ui/src/tokens.css` 가 소유한다. **아래가 현재 전부다** (`001-tokens`,
`TxButton` 2차에서 확정). 값은 이행 전 팔레트와 같게 잡았다 — CSS 로 옮기면서 겉모습까지 바꾸지 않는다.
...
:root {
  ...
  --tx-color-muted: #e5e7eb; /* 표면이 있는 보조 요소 */
  --tx-color-text: #111827;
  --tx-color-on-accent: #ffffff; /* 강조색 위에 얹히는 글자 */

  /* 상태 레이어 — hover·pressed 는 색이 아니라 비율이다. §5-1 */
  --tx-color-state: #111827;
  --tx-state-hover: 16%;
  --tx-state-pressed: 28%;

  --tx-radius: 0.375rem;
  --tx-focus-ring: 2px solid var(--tx-color-primary);
  --tx-focus-ring-offset: 2px;
}

.tx-button {
  /* 컴포넌트 — 전역을 받아쓰되 따로 덮을 수도 있다 */
  --tx-button-bg: var(--tx-color-primary);
  --tx-button-fg: var(--tx-color-on-accent);
  background-color: var(--tx-button-bg);
...
## 5-1. 상태 색은 **토큰이 아니라 파생이다** (2026-08-26)

**`hover` · `pressed` 색을 토큰으로 두지 않는다.** 배경색에 `--tx-color-state` 를 비율만큼 섞어 만든다.
...
역할마다 `--tx-color-<role>-hover` 짝을 두면 소비자가 **둘을 맞춰서** 바꿔야 한다. 하나를 빠뜨리면
버튼은 보라인데 마우스를 올리면 파랑으로 돌아간다. 에러도 경고도 없이 그냥 어긋난다.

> **이건 실제로 났던 결함이다** (`TxButton` D8). 그리고 1차 게이트에서 잡은
> "`className="bg-yellow-500"` 만 주면 평상시만 노랑" 과 **같은 모양**이다 —
> 메커니즘만 Tailwind 에서 CSS 변수로 바뀌고 증상이 그대로 돌아왔다.
> 스파이크에서 발견했다: [TxButton §19](components/02_TxButton.md)

### 규칙

- **variant 는 배경과 글자만 정한다.** 상태 색을 다시 적지 않는다 — 적는 순간 그 variant 만 파생에서 빠진다
- **소비자가 새 variant 를 만들 때도 배경 한 줄이면 된다.** 그게 이 구조의 값이다
- 섞는 색(`--tx-color-state`)은 **다크에서 뒤집고 비율은 그대로 둔다.** 라이트는 진해지고 다크는 옅어진다
- 비율을 바꾸고 싶으면 `--tx-state-hover` 하나다. 역할 수와 무관하다

### 대가

|                             |                                                                                                       |
| --------------------------- | ----------------------------------------------------------------------------------------------------- |
| **`color-mix()` 의존**      | 지원 하한이 **2023년 이후 브라우저**(Chrome 111 · Safari 16.2 · Firefox 113)로 정해진다               |
| 색을 정밀하게 못 고른다     | 팔레트 램프(`blue-500`→`600`)는 채도까지 바꾸는데 파생은 못 한다. **어느 비율로도 그 색은 안 나온다** |
| 비율은 눈으로 맞춘다        | 값 하나가 variant 마다 다르게 보인다 — 밝은 회색이 근검정에서 멀어 더 크게 움직인다. **재고 정한다**  |
| 극단적인 배경은 안 움직인다 | 아래 참고                                                                                             |

역할이 늘어날수록 유리해진다 — 역할 하나당 토큰이 **2개에서 1개**가 되고, 새 variant 는 0개다.

### 섞는 색과 배경이 가까우면 상태 표시가 사라진다

라이트 모드에서 배경이 아주 어두우면 근검정을 섞어도 값이 거의 안 바뀐다.
**`#0f172a` 배경은 hover 가 배경과 완전히 같게 나온다 (ΔL 0).** 다크 모드의 아주 밝은 배경도 같다.

소비자가 어두운 브랜드색을 쓰는 건 흔한 일이라 **탈출구를 규약으로 둔다** — 그 자리만 섞는 색을 뒤집는다.

```css
.tx-button[data-variant="brand"] {
  --tx-button-bg: #0f172a;
  --tx-button-fg: #fff;
  --tx-color-state: #fff; /* 어두운 배경이라 밝은 색을 섞는다 */
}
```

한 줄로 ΔL 0 → 31 이 된다. `R4` 검증에서 실측했다 ([10_requirements §3](10_requirements.md)).
`--tx-color-state` 는 상속되는 값이라 **어느 선택자에 걸든 그 아래로만 바뀐다.**
...
```css
.dark {
  --tx-color-muted: #374151;
  --tx-color-text: #f3f4f6;
}
```

**재정의는 `tokens.css` 의 `.dark` 블록 한 곳에서만 한다.** 거기에는 `--tx-*` 선언 말고
아무것도 들어가지 않는다 — 일반 선언이 섞이기 시작하면 "다크모드 전용 스타일" 이 다시 생긴다.
...
> **전역 토큰의 실제 색값은 이제 §5 가 소유한다.** `TxButton` 2차에서 정해졌다 (2026-08-26).
> 늘리는 규칙은 그대로다 — **쓰는 컴포넌트가 생길 때** 한 줄씩 늘린다.
