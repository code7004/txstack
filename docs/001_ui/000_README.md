...
**49개가 끝났다. 컴포넌트 테스트 1,567개.** 번호는 **만든 차례**다 —
...
| 005  | [`TxIcons`](005_TxIcons.md) _(내부)_             | 내부 전용 아이콘                                    | 18     |
...
| 011  | [`TxPopup`](011_TxPopup.md) _(내부)_             | 앵커에 붙어 뜨는 층                                 | 0      |
...
| 048  | [`TxNavBar`](048_TxNavBar.md)                    | 가로로 도는 내비게이션 · 메가메뉴                   | 26     |
| 049  | [`TxSideNav`](049_TxSideNav.md)                  | 세로로 서는 내비게이션 · 아이콘 rail · 하위 트리    | 27     |

_(내부)_ 는 배럴에서 내보내지 않는 부품이다 — 소비자가 이름을 볼 수 없다.
`/daypicker` `/aggrid` 는 서브패스 전용이다.

## 아직 없는 것

**없다.** 만들 컴포넌트는 다 들어왔다. 남은 일은 **쓰기 쉽게 설명하는 것** —
스토리북을 자세한 사용 안내로 다듬는다. 상태와 다음 순서는 [docs/README](../README.md) 가 갖는다.
...
| `TxTable` (가벼운 표)   | **후보였다가 접었다.** ag-grid 없이 도는 읽기 전용 표를 두면 무게(gzip 485KB)를 아끼고 다크모드가 토큰을 따라오지만, **실제 화면은 대부분 편집·정렬을 쓴다** — 표가 둘이면 어느 쪽을 쓸지부터 고민거리가 된다. `TxAgGrid` 하나로 가고, **대신 그것을 쉽게 쓰는 법을 스토리북이 자세히 안내한다** |
