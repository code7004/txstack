...
1. **`TxTable`** ← 여기서 이어받는다. **어디까지가 `TxAgGrid` 몫인지**를 정해야 해서
   맨 뒤로 미뤄 둔 것이다. `columns` 를 ag-grid `colDef` 의 부분집합으로 잡을지가 첫
   갈림길이고, `ag-grid-community` 가 MIT 인 것은 확인해 뒀다

**원본이 없다** — 이식이 아니라 새로 만드는 것이라 "원본 읽기" 단계가 없다.
