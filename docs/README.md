...
| `ui`          | **컴포넌트 47개 완료** — 목록·진행은 [001_ui](001_ui/000_README.md). 컴포넌트 테스트 1,514개. 남은 둘은 아래 "다음 할 일" |
...
1. **메가메뉴(`TxNavBar`)** ← 여기서 이어받는다. **`TxAppShell` 이 `header` 를 이미
   가졌다** — 무엇이 셸 몫이고 무엇이 남는지부터 정한다. 드로어는 `TxSlidePanel`, hover 메뉴는 `TxDropMenu` 를 그대로 쓴다
2. **`TxTable`** — **어디까지가 `TxAgGrid` 몫인지**를 정해야 해서 맨 뒤다. `columns` 를
   ag-grid `colDef` 의 부분집합으로 잡을지가 첫 갈림길이고, `ag-grid-community` 가
   MIT 인 것은 확인해 뒀다

둘 다 **원본이 없다** — 이식이 아니라 새로 만드는 것이라 "원본 읽기" 단계가 없다.
