...
| `ui`          | 기반 4개 + Form 7개 + `TxPopup` — 테스트 330개 통과                      |
...
1. **Form 클러스터** — `TxCombobox` 까지 끝났다. **`TxDayPicker`(394줄) 가 다음 차례.** ← 여기
   `@txstack/ui/daypicker` 서브패스라 **optional peer 분리를 처음으로 실제 검증**한다.
   `TxInputLike` 를 쓰는 쪽이기도 하다. 그다음이 마지막인 `TxForm`.
