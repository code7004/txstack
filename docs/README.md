...
| `axios`       | **완료** — 테스트 29개 통과. `apps/site` 의 `Example` 이 공개 API 를 실제로 부른다                                                                                                                |
| `hooks`       | **완료** — 테스트 25개 통과. `useUrlQuery` 는 `apps/site` 의 검색 화면이 쓴다                                                                                                                |
...
1. **스토리북을 사용 안내로 다듬는다** ← 여기서 이어받는다. 부품마다 "무엇을 할 수 있는지" 는
...
2. **`apps/site`** ← **지금 여기** — 소개 사이트이자 **네 패키지의 첫 진짜 소비자**다.
   뼈대(A) · 내용(B) · 진짜 데이터(C)까지 섰고 **배포(D)** 가 남았다.
