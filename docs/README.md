...
| `ui`          | 기반 4개 + `TxIcons` + `TxInput` — 테스트 166개 통과                     |
...
1. **Form 클러스터** — `TxIcons` · `TxInput` 이 끝났다. **`TxTextarea` 가 다음 차례.** ← 여기
   이어서 `TxCheckBox` → `TxCapsLockCheck` → `TxDropdown` → `TxDayPicker` → `TxForm`.
   `TxInput` 이 CSS 이행의 본보기다 — 래퍼가 껍데기를 소유하고 `<input>` 은 투명하다.
