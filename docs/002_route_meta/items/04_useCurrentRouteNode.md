...
```
docs/README.md 와 docs/002_route_meta/items/04_useCurrentRouteNode.md 를 읽고 002-useCurrentRouteNode-S1 부터 진행해줘.
```
...
| 단계 | 내용                                            | job ID                       | 상태 | 비고                                              |
| ---- | ----------------------------------------------- | ---------------------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                 | `002-useCurrentRouteNode-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                     | `002-useCurrentRouteNode-S2` |      |                                                   |
| `S3` | 테스트 🤖                                       | `002-useCurrentRouteNode-S3` |      |                                                   |
| `S4` | 스토리북                                        | —                            | —    | `ui` 패키지가 아니므로 해당 없음                  |
| 🧑   | **사용자 확인** — playground·예제로 직접 써본다 | —                            | —    | 통과하면 S5 로. 고칠 게 나오면 S2~S3 을 다시 돈다 |
| `S5` | 문서 사이트 (예제·코드) 🤖                      | `002-useCurrentRouteNode-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                                | `002-useCurrentRouteNode-S6` |      |                                                   |

## 1. 현재 코드 인벤토리 (2026-08-25)

| 항목      | 값                                    |
| --------- | ------------------------------------- |
| 위치      | `packages/route-meta/src/hooks.ts:26` |
| 코드 행수 | 102행                                 |
| export    | `useCurrentRouteNode`                 |

### 착수 전에 알아야 할 것

- 현재 위치가 어느 라우트 노드인지 런타임 매칭
- 102행 — 이 패키지에서 가장 크다. 매칭 규칙을 명세로 뽑아야 한다
