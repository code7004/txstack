...
```
docs/README.md 와 docs/002_route_meta/items/02_getNavigableRoutes.md 를 읽고 002-getNavigableRoutes-S1 부터 진행해줘.
```
...
| 단계 | 내용                                            | job ID                      | 상태 | 비고                                              |
| ---- | ----------------------------------------------- | --------------------------- | ---- | ------------------------------------------------- |
| `S1` | 문서 = 명세 + 현행 코드 감사 🤝                 | `002-getNavigableRoutes-S1` |      |                                                   |
| `S2` | 구현 = 감사 결과 반영 🧑/🤖                     | `002-getNavigableRoutes-S2` |      |                                                   |
| `S3` | 테스트 🤖                                       | `002-getNavigableRoutes-S3` |      |                                                   |
| `S4` | 스토리북                                        | —                           | —    | `ui` 패키지가 아니므로 해당 없음                  |
| 🧑   | **사용자 확인** — playground·예제로 직접 써본다 | —                           | —    | 통과하면 S5 로. 고칠 게 나오면 S2~S3 을 다시 돈다 |
| `S5` | 문서 사이트 (예제·코드) 🤖                      | `002-getNavigableRoutes-S5` |      |                                                   |
| `S6` | Claude 가이드 🤖                                | `002-getNavigableRoutes-S6` |      |                                                   |

## 1. 현재 코드 인벤토리 (2026-08-25)

| 항목      | 값                                    |
| --------- | ------------------------------------- |
| 위치      | `packages/route-meta/src/utils.ts:65` |
| 코드 행수 | 94행                                  |
| export    | `getNavigableRoutes`                  |

### 착수 전에 알아야 할 것

- **시그니처가 `(tree, permission?)` 다.** 패키지가 권한 개념을 알고 있다 — [01_ARCHITECTURE](../../00_foundation/01_ARCHITECTURE.md) 의 "런타임 정책을 패키지가 결정하지 않는다" 와 충돌하는지 판단
- `scope` 가 권한인지 메뉴 표시 범위인지가 여기서 드러난다
