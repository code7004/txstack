    ...
    docs: {
      description: {
        component: [
          ...
          "",
          "**두 아이콘은 진짜 `<button>` 이다.** 원본은 SVG 에 `onClick` 을 걸어서 키보드로 도달할 수 없었고,",
          "스크린리더가 버튼으로 읽지 않았다 — 검색과 지우기를 마우스로만 쓸 수 있었다.",
          "Tab 으로 이동해 보면 포커스 링이 각 버튼에 걸린다.",
          "",
          "안쪽 `TxInput` 의 테두리는 **CSS 가 지운다** (`.tx-search-input .tx-input`).",
          "원본은 `theme` prop 으로 클래스를 비워 지웠는데 그 prop 은 폐기됐다.",
          "",
 ...
 *
 * 원본은 표시 여부를 별도 state 로 들어서, `value` prop 으로 값을 넣으면 버튼이 안 나왔다.
...
/** `TxInput` 과 나란히 놓았을 때 높이와 테두리가 맞는지 본다. 껍데기를 같은 토큰으로 그린다. */
