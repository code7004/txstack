    ...
    docs: {
      description: {
        component: [
          ...
          "```",
          "",
          "- **바깥을 눌러도 닫히지 않는다.** 네이티브 `alert` · `confirm` 처럼 답하기 전에는 안 닫힌다 —",
          '  `confirm` 에서 바깥을 잘못 누르면 조용히 "취소를 골랐다" 가 되어 버린다',
          "- **취소 · Escape 는 `false`** 다. 네이티브 `confirm` 과 같다",
...
/**
 * 예/아니오를 받는다. **취소 · Escape 가 `false`** 다.
 *
